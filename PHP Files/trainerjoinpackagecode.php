...
$data=mysqli_query($con,"select * from newtrainer where email='$ab'")or die(mysqli_error($con));
...
$data=mysqli_query($con,"select * from addpackage where id='$aa'")or die(mysqli_error($con));
	...
	if(mysqli_num_rows($data)>0){
		header("location:trainerallpackage.php?a=1");
	}
	else{
	...
	mysqli_query($con,"insert into joinpackage(user,email,mobile,name,price,duration,offer,pid,sdate,tdays,status) values('$user','$email','$mobile','$name','$price','$duration','$offer','$pid','$date','$days','active')") or die(mysqli_error($con));
